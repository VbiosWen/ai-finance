...
## Corpus Check
- 148 files · ~37,902 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1069 nodes · 2019 edges · 101 communities (72 shown, 29 thin omitted)
- Extraction: 71% EXTRACTED · 29% INFERRED · 0% AMBIGUOUS · INFERRED: 594 edges (avg confidence: 0.6)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `307ad19c`
...
- ConversationStatus
- SkillRef
- TestNacosSkillConfigRepository
- README.md
...
Cohesion: 0.06
Nodes (27): 识别当前对话最匹配的技能。基础设施层用 LLM 结构化输出实现。, 路由 Agent 服务 —— 实现 AgentService 端口，内敛路由复杂度。  编排：意图识别 → RoutingPolicy 裁决 → 分发到选中的技, _routing_message(), 置信度阈值裁决：低置信/无匹配/不在可路由集 → 兜底。, IntentClassification, BaseModel, 意图识别与路由裁决的值对象（不可变，零框架依赖）。, 路由裁决结果——由 RoutingPolicy 产出。 (+19 more)

### Community 1 - "Conversation Use Case & Persistence"
Cohesion: 0.06
Nodes (34): ChatResult, BaseModel, 对话用例的命令与结果 DTO（Pydantic v2）。, SendMessageCommand, 发送消息用例 —— 对话闭环编排。业务规则在 domain，此处只编排。, SendMessageUseCase, DomainEventPublisher, Protocol (+26 more)

### Community 2 - "Agent Entity, DTO & Service Ports"
Cohesion: 0.16
Nodes (8): BaseModel, SkillConfig, NacosSkillConfigRepository, SkillConfig 的 Nacos 适配器 —— 实现 domain 层的 SkillConfigRepository 端口。  从 Nacos YAML, 从 Nacos 读取 SkillConfig 列表，支持热更新。      用法::          repo = NacosSkillConfigRepos, 从 Nacos 拉取并注册 watcher（幂等，并发安全）。, LoadedSkillRepo, 预置缓存的 SkillConfig 仓库桩(不连 Nacos)。

### Community 3 - "HTTP SSE Streaming & Agent Router"
Cohesion: 0.08
Nodes (20): AIMessage, HumanMessage, _build_config(), _count_tool_calls(), _extract_last_ai_content(), Any, LangChain Agent 适配器——实现 AgentService 端口。  将基础设施层装配的 LangChain ReAct Agent，包装为 ap, 将 Agent DTO 消息列表转为 LangChain Message 对象列表。 (+12 more)

### Community 4 - "Infrastructure AI Tools & Adapters"
Cohesion: 0.14
Nodes (11): P, AgentIdentity 配置仓储端口。  定义 Agent 身份定义配置的访问契约，由基础设施层 Nacos 适配器实现。, ConfigRepository, ABC, 配置仓储泛型端口。  定义了从外部配置源（Nacos、本地文件等）读取和监听配置变更的通用抽象。 具体业务配置（AgentIdentity、SkillConfi, 配置仓储抽象基类。      定义 key-value 风格的配置访问模式，每个具体仓储负责一种配置类型。      Type Parameters:, 按键查询配置。          Args:             key: 配置键。          Returns:             对应类型的, 注册配置变更回调。          当远程配置发生热更新时，所有已注册的回调会按序触发。          Args:             callbac (+3 more)

### Community 5 - "Agent Config API Routes"
Cohesion: 0.12
Nodes (12): AgentService, Protocol, AI Agent 服务端口——定义 Agent 调用的抽象契约。  应用层依赖此端口，基础设施层提供 LangChain 实现。, Agent 服务抽象端口。      定义 Agent 的标准调用方式：     - run: 同步/非流式调用，返回完整结果。     - stream: 异, AgentRegistry, 多技能 Agent 注册表 —— 分类器目录与可路由集合同源。, skill_name → (SkillConfig, AgentService)。, AgentServicePortTest (+4 more)

### Community 6 - "Conversation REST Router & Schemas"
Cohesion: 0.12
Nodes (12): AgentStreamEvent, Agent 流式事件。      支持六种事件类型：     - token: LLM 输出的增量文本片段。     - tool_start: 工具开始执行。, 流式调用 Agent，逐事件推送。          Args:             request: 包含消息列表和可选 thread_id 的请求。, LangChainAgentService, _map_event(), 将 LangChain astream_events 事件映射为 AgentStreamEvent。, 基于 LangChain 的 AgentService 实现。      封装 LangGraph 编译后的 agent，实现 run 与 stream 两种调, 流式调用 Agent，逐事件推送（适用 SSE）。 (+4 more)

### Community 7 - "Nacos Config Repository Ports"
Cohesion: 0.17
Nodes (8): AgentIdentity, BaseModel, NacosAgentIdentityRepository, AgentIdentity 的 Nacos 适配器 —— 实现 domain 层的 AgentIdentityRepository 端口。  从 Nacos Y, 从 Nacos 读取 AgentIdentity，支持热更新。      用法::          repo = NacosAgentIdentityRepo, 从 Nacos 拉取并注册 watcher（幂等，并发安全）。, LoadedIdentityRepo, 预置缓存的 AgentIdentity 仓库桩(不连 Nacos)。
...
Cohesion: 0.23
Nodes (7): Conversation, BaseModel, 关闭对话（幂等），发 ConversationClosed。, 追加用户消息。不变量：status == ACTIVE 才能追加。, 追加助手回复，发 AssistantResponded。, Message, 一条对话消息——一旦产生不可改（审计完整性）。

### Community 10 - "Domain Shared AI Tools"
Cohesion: 0.17
Nodes (5): 工具注册表——全局 AITool 实例池。  启动时注册所有业务工具，运行时按名称解析，供 SkillAgentFactory 为每个 Skill 动态装配专属, 全局工具注册表。      以工具名称（AITool.name）为键存储所有可用工具实例。     SkillAgentFactory 根据 SkillConf, 注册一个工具实例。          Args:             tool: 实现了 AITool 端口的工具实例。          Raises:, 按名称列表解析工具实例。          Args:             names: 工具名称列表。          Returns:, ToolRegistry

### Community 11 - "Domain Value Objects (Agent Identity, Skill)"
Cohesion: 0.22
Nodes (6): 装配该 Agent 的工具链。          若 prompt_config 使用了 skill_refs（动态模式），自动注入         ``Ski, 领域工具包——工具注册表、技能查询工具等。, 技能查询工具 —— 让 LLM 按需获取技能完整指令。  作为 Router Agent 的元工具，LLM 在选择技能后调用此工具获取 该技能的 task_in, 按名称查询技能详情。      在 Router Agent 中注册，让 LLM 先看菜单再获取完整指令。     避免把所有技能的详细 prompt 全部塞进, 查询技能详情。          Args:             **kwargs: 应包含 ``skill_name``（要查询的技能名）。, SkillLookupTool
...
Cohesion: 0.16
Nodes (9): AgentResponse, BaseModel, Agent 数据传输对象（DTO）。  使用 Pydantic v2 定义 agent 调用的输入输出模型, 仅限 application 和 interfac, Agent 非流式响应。      Attributes:         reply: Agent 的最终回复文本。         thread_id: 关, 非流式调用 Agent，返回完整结果。          Args:             request: 包含消息列表和可选 thread_id 的请求。, AgentResponseTest, Agent DTO 测试——验证 Pydantic 模型的序列化与校验。, AgentResponse DTO 测试。 (+1 more)
...
Cohesion: 0.25
Nodes (3): 结构性实现 AgentService 端口的桩。, StubAgentService, ConversationRouterTest
...
Cohesion: 0.12
Nodes (7): _nacos_is_available(), NacosClient 单元测试。  需要 Docker 中运行 Nacos 才能通过集成测试。 若 Nacos 不可用，集成测试会自动跳过。, 检测本地 Nacos gRPC 端口 9848 是否可达。, NacosAgentIdentityRepository 集成测试。, TestNacosAgentIdentityRepository, TestNacosClientLifecycle, TestNacosClientRead
...
Cohesion: 0.15
Nodes (14): Exception, build_container(), 构建并返回组合根容器(async 全链装配)。      任一步失败时,AsyncExitStack 逆序回收已启动资源后异常上抛(fail-fast)。, LLMClientFactory, Any, build_agent_registry(), _build_one(), 多技能 Agent 装配 —— 从 SkillConfig 列表建「每技能一个 Agent」+ 兜底 general。 (+6 more)
...
Cohesion: 0.13
Nodes (8): create_react_agent(), Any, ReAct Agent 工厂——基于 LangChain create_agent 组装 Agent。  LangChain 1.3.13 中 create_r, 创建 ReAct Agent。      使用 LangChain 的 create_agent API，底层基于 LangGraph StateGraph，, TestCreateAgent, LLM 客户端工厂测试——验证 LLMClientFactory 的创建逻辑。, LLMClientFactory 单元测试。, TestLLMClientFactory
...
Cohesion: 0.19
Nodes (6): ConversationId, BaseModel, 对话标识（uuid7 hex，32 位小写十六进制，趋势递增）。, 生成新标识——单调 uuid7，ID 字典序即创建时间序。, ToolCallRecord, ValueObjectsTest

### Community 32 - "Agent Service Port Tests"
Cohesion: 0.18
Nodes (8): AgentRequest, Agent 调用请求。      Attributes:         messages: 对话消息列表，每条含 role 和 content。, IntentRecognizer, Protocol, 意图识别端口 —— 输入对话 + 技能目录，输出结构化分类结果。, 实现 AgentService 端口（run / stream）。, RoutingAgentService, AgentRequestTest

### Community 33 - "Test Support Fixtures"
Cohesion: 0.15
Nodes (14): AsyncExitStack, configure_logging(), create_app(), FastAPI, FastAPI 应用工厂与生命周期。  create_app() 创建应用:注册 lifespan 与 interfaces 层路由。 生产路径(contain, 幂等的全局日志配置(root 已有 handler 时 basicConfig 不生效)。, 创建 FastAPI 应用。      Args:         container: 预制容器(测试用),由调用方负责 shutdown;, make_stub_container() (+6 more)
...
Cohesion: 0.22
Nodes (10): Agent SSE 流式输出实现计划, HTTP 请求/响应 Schema (ChatRequest/ChatResponse/ChatMessage), 对话 (Conversation) 子域实现计划, Agent 动态路由与意图识别实现计划, AgentService 应用级单例 (lifespan 预热), Agent SSE 流式输出设计, SSE 线上协议 (text/event-stream), sse-starlette EventSourceResponse 心跳+断连检测 (+2 more)
...
Cohesion: 0.29
Nodes (11): AgentStreamEvent 六类事件 (token/tool_start/tool_end/done/error/routing), AgentRegistry 多技能注册表, GENERAL_SKILL 兜底通用技能, IntentClassification 值对象 (target_skill+confidence+reason), IntentRecognizer 意图识别端口, Agent 动态路由与意图识别设计, LlmIntentRecognizer 结构化输出实现, RoutingAgentService 路由分发编排 (+3 more)
...
Cohesion: 0.29
Nodes (4): 组合根:集中装配各层依赖。  build_container() 一次性完成全链装配(Nacos → 配置仓库 → 数据库 → Agent), 产出不可变 Co, 测试共享桩:构造零 IO 的 Container。  Container 字段类型是具体基础设施类(Pydantic isinstance 校验),因此桩必须是, Container 构造、不可变性与关闭语义测试——全部零 IO 桩,不触网络/数据库。, 对话路由集成——注入 stub 用例，验证 /conversations/messages。

### Community 42 - "Agent Router Tool Domain"
Cohesion: 0.29
Nodes (5): AgentPromptConfig, BaseModel, Agent 提示词配置 —— 不可变值对象。  组合 AgentIdentity + SkillRef/SkillConfig + 运行时参数， 为一个 Age, Agent 的一整套提示词配置（不可变）。, 渲染完整系统提示词。          两种模式：         - **动态模式**（推荐）：若 ``skill_refs`` 非空，仅渲染技能菜单，
...
Cohesion: 0.17
Nodes (11): 路由裁决领域服务 —— 无状态，脱离 LLM 可单测。, RoutingPolicy, BaseModel, RoutingConfig, RoutingConfigTest, RoutingAgentService 测试——路由分发、routing 首帧、失败降级、run 回填。, RoutingAgentServiceTest, _service() (+3 more)
...
### Community 88 - "uuid7"
Cohesion: 0.23
Nodes (5): 进程内单调 UUIDv7 生成器（RFC 9562, Method 1）。  纯标准库实现，零第三方依赖——供各子域生成趋势递增的聚合标识。 单调性：同进程严格, uuid7(), IdGeneratorTest, 单调 uuid7 生成器测试——格式、位段、严格递增、并发唯一、回拨不倒退。, UUID
...
### Community 91 - "AgentIdentityRepository"
Cohesion: 0.20
Nodes (7): AgentIdentityRepository, Agent 身份定义的配置仓储。      每个 key 对应一个 ``AgentIdentity`` 实例。, 获取指定 key 的 Agent 身份定义。          Args:             key: 配置键（如 ``"agent-identity"`, 注册 AgentIdentity 变更回调。          Args:             callback: 接收新的 AgentIdentity 实, agent_identity(), agent_skills(), Agent 配置只读路由——身份定义与技能列表。

### Community 92 - "AITool"
Cohesion: 0.22
Nodes (6): AITool, ABC, AI 工具端口——定义工具契约。  领域层不依赖任何框架（如 LangChain）。 具体工具的语义（做什么）在领域层定义； 工具的 LangChain 包装（, AI 工具端口（抽象基类）。      每一个领域工具需实现此接口，定义：     - name: LLM 用于识别的工具名称。     - descripti, 工具描述。LLM 阅读此文本决定何时调用工具。          建议包含：功能说明、适用场景、参数含义。, 执行工具逻辑。          Args:             **kwargs: LLM 传入的参数（字符串键值对），由具体工具自行解析和校验。
...
Cohesion: 0.25
Nodes (7): DispatchToSkillTool, _extract_last_ai_content(), Any, 技能分派工具 —— 将用户任务转发给指定 Skill 的专属子 Agent 执行。  作为 Router Agent 的执行工具，LLM 在查询技能详情后调用此, 从 LangGraph 消息列表中提取最后一条 AI 消息的文本内容。, 将任务转发给指定技能的专属子 Agent 执行。      子 Agent 持有该技能专属的 tools 和完整 prompt，Router Agent, 执行技能任务。          Args:             **kwargs: 应包含 ``skill_name``（技能名）和 ``task``（任
...
### Community 96 - "SkillConfigRepository"
Cohesion: 0.25
Nodes (5): 技能配置仓储。      每个 key 对应一个 ``SkillConfig`` 列表（一个 Agent 可关联多个技能）。, 获取指定 key 的技能配置列表。          Args:             key: 配置键（如 ``"skill-configs"``）。, 注册技能配置变更回调。          Args:             callback: 接收新的 SkillConfig 列表的回调函数。, SkillConfigRepository, 初始化分派工具。          Args:             agent_factory: 子 Agent 工厂（负责编译和缓存）。

### Community 97 - "ConversationStatus"
Cohesion: 0.43
Nodes (5): Enum, 仓储重建：装入历史消息，不发事件、不入 pending。, ConversationStatus, MessageRole, str

### Community 98 - "SkillRef"
Cohesion: 0.29
Nodes (5): 初始化技能查询工具。          Args:             skill_repo: 技能配置仓储（含完整 SkillConfig）。, BaseModel, 技能轻量引用 —— 不可变值对象。  与 ``SkillConfig`` 不同，SkillRef 仅含技能的标识与一句话简介， 不包含完整指令。用于 Agent, 技能的轻量引用（名称 + 简介）。      LLM 阅读此引用判断是否需要该技能，需要时通过 ``lookup_skill`` 工具     查询 ``Ski, SkillRef

## Knowledge Gaps
- **29 isolated node(s):** `ai-finance`, `ai-finance`, `Task 1: 单调 uuid7 生成器`, `Task 2: ConversationId 加固（pattern 校验 + new() 工厂）`, `Task 3: 聚合工厂切换到单调 ID` (+24 more)
...
- **Why does `Container` connect `Bootstrap App Factory Tests` to `Agent Service Port Tests`, `Test Support Fixtures`, `Conversation Use Case & Persistence`, `Agent Entity, DTO & Service Ports`, `Intent Routing & Agent Registry`, `Agent Config API Routes`, `Nacos Config Repository Ports`, `LLM Client Factory Infrastructure`, `Bootstrap DI & Container Tests`, `Nacos Client Infrastructure`, `LLM Config & Client Factory`, `Bootstrap Container Tests`, `LLM Client Factory Tests`, `Nacos Repository Implementation Tests`, `Agent Factory & Builder Tests`, `Tool Adapter Tests`, `AITool`?**
  _High betweenness centrality (0.193) - this node is a cross-community bridge._
- **Why does `AgentRequest` connect `Agent Service Port Tests` to `Conversation Use Case & Persistence`, `Agent Entity, DTO & Service Ports`, `HTTP SSE Streaming & Agent Router`, `Agent Config API Routes`, `Conversation REST Router & Schemas`, `Nacos Config Repository Ports`, `Agent HTTP Router Interface`, `Agent Router Tool Tests`, `Agent Service Test Support`, `Bootstrap Container Tests`, `End-to-End Integration Tests`, `LLM Client Factory Tests`, `Nacos Client Lifecycle Tests`?**
  _High betweenness centrality (0.125) - this node is a cross-community bridge._
- **Why does `AgentId` connect `Conversation Use Case & Persistence` to `SkillConfigRepository`, `ConversationStatus`, `Conversation Value Objects`, `Agent Router Tool Domain`, `Domain Value Objects (Agent Identity, Skill)`, `Skill Config & Agent Builder`, `AITool`, `Skill Lookup & Tool Registry`, `LLM Intent Recognizer Tests`, `Routing Policy Tests`, `Domain Prompts & General Skills`?**
